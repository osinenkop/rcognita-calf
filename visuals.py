...
class Animator3WRobot(Animator):
    ...
    def animate(self, k):
        ...
        if self.is_print_sim_step:
            self.logger.print_sim_step(t, xCoord, yCoord, alpha, v, omega, run_obj, accum_obj, action)
            
        if self.is_log_data:
            self.logger.log_data_row(self.datafile_curr, t, xCoord, yCoord, alpha, v, omega, run_obj, accum_obj, action)
...
class Animator3WRobotNI(Animator):
    ...
    def __init__(self, objects=[], pars=[]):
        ...
        # ###################################################################################################
        ###################################################################################################
        circle_3 = plt.Circle((circle_coord[0], circle_coord[1]), radius=3*circle_coord[2], color='pink', fill=True, lw=2)
        self.axs_xy_plane.add_artist(circle_3)        
        circle_3 = plt.Circle((circle_coord[0], circle_coord[1]), radius=2*circle_coord[2], color='lightpink', fill=True, lw=2)
        self.axs_xy_plane.add_artist(circle_3)
        circle_3 = plt.Circle((circle_coord[0], circle_coord[1]), radius=1*circle_coord[2], color='r', fill=True, lw=2)
        self.axs_xy_plane.add_artist(circle_3)
    ...
    def animate(self, k):
        ...
        count_CALF = self.ctrl_benchmarking.get_CALF_count()
        count_N_CTRL = self.ctrl_benchmarking.get_N_CTRL_count()
        ...
        if self.is_print_sim_step:
            self.logger.print_sim_step(t, xCoord, yCoord, alpha, run_obj, accum_obj, action, count_CALF, count_N_CTRL)
            
        if self.is_log_data:
            self.logger.log_data_row(self.datafile_curr, t, xCoord, yCoord, alpha, run_obj, accum_obj, action, count_CALF, count_N_CTRL)
        ...
        # Run done
        if self.ctrl_mode == "SARSM":
            if t >= self.t1 or (np.linalg.norm(observation[:2])) < 0.2:# and abs(observation[2]) < 0.1): 
                if self.is_print_sim_step:
                    print('.....................................Run {run:2d} done.....................................'.format(run = self.run_curr))  
                    
                self.run_curr += 1
    
                # Вычислим средние значения координат для первой линии
                avg_x1 = self.line_traj.get_xdata()
                avg_y1 = self.line_traj.get_ydata()
                index = int((len(avg_x1-1) - self.index_prev)/7)
                index = self.index_prev + index
                self.index_prev = len(avg_x1-1)
                # self.axs_xy_plane.annotate('{} run'.format(self.run_curr-1), xy=(avg_x1[index], avg_y1[index]), textcoords="offset points", xytext=(0,10), ha='center', va='bottom', arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))
...
class Animator2Tank(Animator):
    ...
    def __init__(self, objects=[], pars=[]):
        ...
        # Pack all lines together
        cLines = namedtuple('lines', ['line_h1', 'line_h2', 'line_run_obj', 'line_accum_obj', 'line_ctrl'])
    ...
    def animate(self, k):
        ...
        if self.is_print_sim_step:
            self.logger.print_sim_step(t, h1, h2, p, run_obj, accum_obj)
            
        if self.is_log_data:
            self.logger.log_data_row(self.datafile_curr, t, h1, h2, p, run_obj, accum_obj)
